...
public OnPluginStart() {
	...
	RegAdminCmd("sm_forceboss", ForceSpawn, ADMFLAG_ROOT, "Forces a boss to spawn");
	RegAdminCmd("sm_slayboss", SlayBoss, ADMFLAG_ROOT, "Forces a boss to die");
...
public cvarChange(Handle:convar, String:oldValue[], String:newValue[]) {
	if(convar == Version) {
		...
		if(StringToInt(oldValue) == 0) {
			...
			&& GetConVarInt(Skeleton) == StringToInt(oldValue)) {
				ResetTimer();
			}
		}
	}
	...
	else if(convar == Skeleton) {
		if(StringToInt(oldValue) == 0) {
			if(GetConVarInt(Horseman) == StringToInt(oldValue) 
			&& GetConVarInt(Monoculus) == StringToInt(oldValue) 
			&& GetConVarInt(Merasmus) == StringToInt(oldValue)
			&& GetConVarInt(Skeleton) == StringToInt(newValue)) {
				ResetTimer();
			}
		}
	}
