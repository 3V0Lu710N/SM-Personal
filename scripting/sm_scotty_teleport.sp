...
public OnPluginStart() 
	...
	RegAdminCmd("sm_send", Beam_Me, ADMFLAG_ROOT, "sm_send <target name> <teleporter name>");
	RegAdminCmd("sm_bring", Beam_Tome, ADMFLAG_ROOT, "sm_bring <teleporter name>");
...
public Action:Beam_Me(client, args) 
	...
	if (args < 2)
	{
		ReplyToCommand(client, "[SM] Usage: sm_send <target name> <teleporter name>");
...
public Action:Beam_Tome(client, args) 
	...
	if (args < 1)
	{
		ReplyToCommand(client, "[SM] Usage: sm_bring <teleporter name>");
		return Plugin_Handled;
	}
	new String:arg[32];
