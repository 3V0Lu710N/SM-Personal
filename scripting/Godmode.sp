...
public OnPlayerSpawned(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	else
	{
		...
		if (GetConVarBool(v_SpawnAdminOnly) && !CheckCommandAccess(client, "godmode_adminonly_override", ADMFLAG_UNBAN, true))
...
{
	if (args != 0 && !CheckCommandAccess(client, "sm_godmode_admin", ADMFLAG_UNBAN, true))
	{
...
{
	if (args != 0 && !CheckCommandAccess(client, "sm_buddhamode_admin", ADMFLAG_UNBAN, true))
	{
...
{	
	if (args != 0 && !CheckCommandAccess(client, "sm_mortalmode_admin", ADMFLAG_UNBAN, true))
	{
