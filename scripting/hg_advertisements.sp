...
new Handle:g_hTimer;
new g_iGameServerType = SERVER_CSS;
new g_iServerType = SERVER_ALL;
...
{
    new Handle:servertype = FindConVar("hg_premium_server_type");
 
    if (servertype == INVALID_HANDLE)
    {
        LogError("Premium loaded late, so we're just going to use the game type.");
    }
    else
    {
        g_iServerType = GetConVarInt(servertype)|g_iGameServerType|SERVER_ALL;
    }
    
    // Add database advertisements
...
public APLRes:AskPluginLoad2(Handle:myself, bool:late, String:error[], err_max)
{
    ...
    g_iServerType |= g_iGameServerType;
...
ParseDatabaseAds()
{
    ...
    Format(query, sizeof(query),
           "SELECT type, text, flags FROM adverts WHERE (servertype & %d) and (duration_seconds = 0 or (datetime_added + duration_seconds) > UNIX_TIMESTAMP()) and (servertype > 0)",
           g_iServerType);

    SQL_TQuery(hDBConn, GrabHGAdverts, query);
}
...
public DBConnectCallback(Handle:driver, Handle:conn, const String:error[], any:data)
{
    ...
    {
        LogError("Unable to connect to %s (%s)", DB_NAME, error);
        CreateTimer(UPDATE_FREQUENCY, ConnectToDB);
...
public GrabHGAdverts(Handle:main, Handle:hndl, const String:error[], any:data)
{
    ...
    decl String:type[256];
    decl String:text[256];
    decl String:flag[256];
    ...
    while (SQL_FetchRow(hndl))
    {
        ...
        Format(keyName, sizeof(keyName), "DB-%d", iRowCount);
        iRowCount++;
         
        KvJumpToKey(g_hAdvertisements, keyName, true);
        KvSetString(g_hAdvertisements, "type", type);
        KvSetString(g_hAdvertisements, "text", text);
        KvSetString(g_hAdvertisements, "flags", flag);
