...
{
	name = "Rocket Launch (Modded by 3V0Lu710N)",
	author = "eva-dog",
	description = "Launch people into SPACEEEEEEE",
...
{
	CreateConVar("sm_rocket_version", PLUGIN_VERSION, " Rocket Version", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY)
	
	RegAdminCmd("sm_rocket", Command_RocketLaunch, ADMFLAG_UNBAN, "sm_rocket <#userid|name>")
...
public Action:Command_RocketLaunch(client, args)
{
	...
	if (args < 1)
	{
		ReplyToCommand(client, "[SM] Usage: sm_rocket <#userid|name>");
	...
	for (new i = 0; i < target_count; i++)
	{
		...
		{
			PerformRocketLaunch(client, target_list[i])
		}
	}
	return Plugin_Handled
}

PerformRocketLaunch(client, target)
{
	if (g_Ent[target] == 0)
	{
		if (client != -1)
		{
			LogAction(client, target, "\"%L\" launched \"%L\" into space", client, target)
			ShowActivity(client, "launched %N", target)
			new String:name[MAX_NAME_LENGTH];
			GetClientName(target, name, sizeof(name));
			CPrintToChatAll("{collectors}[HG] {selfmade}%s {default}-{darkorange} GOT LAUNCHED INTO {black}S{white}P{black}A{white}C{black}E{white}E{black}E{white}E{black}E{white}E", name)
...
public AdminMenu_rocket( Handle:topmenu, TopMenuAction:action, TopMenuObject:object_id, param, String:buffer[], maxlength )
{
	...
	{
		Format(buffer, maxlength, "Evo's Rocket Launch")
	}
...
public MenuHandler_Players(Handle:menu, MenuAction:action, param1, param2)
{
	...
	else if (action == MenuAction_Select)
	{
		...
		{					
			PerformRocketLaunch(param1, target)
		}
