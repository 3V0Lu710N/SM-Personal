...
public OnPluginStart()
{
    ...
    RegAdminCmd("sm_setcash", Command_SetCash, ADMFLAG_SLAY, "Set target(s) cash for Mann vs Machine mode, Usage: sm_setcash \"target\" \"amount\"");
    ...
    RegAdminCmd("sm_removecash", Command_RemoveCash, ADMFLAG_UNBAN, "Add to target(s) cash for Mann vs Machine mode, Usage: sm_setcash \"target\" \"amount\"");
...
public Action:Command_AddCash(client, args)
{
    if(args != 1)
    {
        ReplyToCommand(client, "[SM] Usage: sm_addcash \"amount\"");
        return Plugin_Handled;
    }

    new String:strCash[32], iCash;
    GetCmdArg(1, strCash, sizeof(strCash));
    iCash = StringToInt(strCash);
    if(iCash < 1)
    {
    ...
    SetCash(client, GetCash(client)+iCash);
    return Plugin_Handled;
}
