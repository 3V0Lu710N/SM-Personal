...
public OnPluginStart()
{
	...
	g_cvars[CVAR_ITEMFLAG] = CreateConVar("falsemessages_itemflag", "a", "Admin flag required for players to use item-related commands.", FCVAR_PLUGIN);
	...
	RegConsoleCmd("sm_fakeitem", Command_FakeItem);
	RegConsoleCmd("sm_fakelose", Command_FakeLose);
	RegConsoleCmd("sm_fakecraft", Command_FakeCraft);
...
public Action:Command_FakeItem(client, args)
{
	...
	if(args < 2)
	{
		ReplyToCommand(client, "[SM] Usage: sm_fakeitem <client> \"Item Name in Quotes\"");
		return Plugin_Handled;
	}
	...
	decl String:log[256];
	Format(log, sizeof(log), "%N used sm_fakeitem \"%s\" on", client, item);
	if (target_count > 1)
	{
	...
	{
		PrintItemMessage(client, item, target_list[i], 1);
	}
...
public Action:Command_FakeCraft(client, args)
{
	...
	decl String:log[256];
	Format(log, sizeof(log), "%N used sm_fakecraft \"%s\" on", client, item);
	if (target_count > 1)
	{
		Format(log, sizeof(log), "%s multiple targets.", log);
	}
	...
	for (new i=0; i<target_count; i++)
	{
		PrintItemMessage(client, item, target_list[i], 3);
	}
