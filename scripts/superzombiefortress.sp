...
public Plugin:myinfo = 
{
    ...
    description     = "Pits a team of survivors aganist an endless onslaught of zombie.",
...
new zf_spawnzombieKilledCounter;
// Client State
...
public Action:hook_JoinClass(client, const String:command[], argc)
{
    ...
    if(isZom(client))     
    {
        ...
        {
            PrintToChat(client, "\x05[ZF]\x01 %t", "Valid zombie: Scout, Heavy, Spy.");
        }
    }
...
public Action:hook_zfTeamPref(client, const String:command[], argc)
{
    ...
    if(argc == 0)
    {
        if(prefGet(client, TeamPref) == ZF_TEAMPREF_SUR)
            ...
            ReplyToCommand(client, "%t", "zombie");
...
{    
    ...
    // + zombie: Crit result is based solely on bonus calculation. 
    if(isSur(client))
    {
...
public Action:event_PlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
    ...
    if(validZom(victim))
    {
        // Remove dropped ammopacks from zombie.
        new index = -1; 
    ...
    {
        if(validSur(killers[0])) zf_spawnzombieKilledCounter--;
...
{ 
    ...
    if(!mapIsZF())
    {
        ...
        while((index = FindEntityByClassname(index, "func_respawnroomvisualizer")) != -1)
        {
            ...
            if (!StrEqual(strParent, "zombiepawn", false))
            {
                AcceptEntityInput(index, "Disable");
            }
        }
...
handle_zombieAbilities()
{
    ...
    for(new i = 1; i <= MaxClients; i++)
    {
        ...
        {     
            ...
            //        zombie regenerate health based on class and number of nearby
            //        zombie (hoarde bonus). zombie decay health when overhealed.
            bonus = 0;
            if(curH < maxH)
            {
                switch(clientClass)
            ...
            //        zombie receive crit bonus based on number of nearby zombie
            //        (hoarde bonus). zombie only receive this bonus at full health
            //        or greater.
            bonus = 0;
            if(curH >= maxH)
            {
...
zfSetTeams()
{
    ...
    // + By default, survivors are RED and zombie are BLU.
    //
...
public panel_PrintPrefs00(client)
{
	...
	{
		Format(temp_string2, sizeof(temp_string2),"%T", "(Current) zombie", client);
        DrawPanelItem(panel, temp_string2, ITEMDRAW_DISABLED);
	}
    else
	{
		Format(temp_string2, sizeof(temp_string2),"%T", "zombie", client);
        DrawPanelItem(panel, temp_string2);
	}  
...
public panel_PrintHelp(client)
{
	...
	DrawPanelItem(panel, temp_string3);
	Format(temp_string3, sizeof(temp_string3),"%T", "Team: zombie", client);
	DrawPanelItem(panel, temp_string3);
	Format(temp_string3, sizeof(temp_string3),"%T", "Classes: Survivors", client);
	DrawPanelItem(panel, temp_string3);
	Format(temp_string3, sizeof(temp_string3),"%T", "Classes: zombie", client);
	DrawPanelItem(panel, temp_string3);
...
public panel_PrintTeam(client, team)
{
    ...
    else if(team == _:zomTeam())
    {
        ...
        DrawPanelText(panel, "-------------------------------------------");
		Format(temp_string6, sizeof(temp_string6),"%T", "zombie consist of scouts, heavies, and", client);
...
UpdateZombieDamageScale()
{
    ...
    if (!g_bZombieRage && g_iZombieTank <= 0 && !zombieHaveTank())
    {
...
Createzombiekin(iClient) {   
	...
	if (TF2_GetPlayerClass(iClient) == TFClass_Scout) Format(strModel, sizeof(strModel), "models/custom/player/zombie/spy.mdl");
...
PrecacheZombieModels() {
    ...
    AddFileToDownloadsTable("materials/models/custom/player/meangreen/hvyweapon/eyeball_invun.vmt");
    AddFileToDownloadsTable("materials/models/custom/player/meangreen/hvyweapon/eyeball_l.vmt");
    AddFileToDownloadsTable("materials/models/custom/player/meangreen/hvyweapon/eyeball_r.vmt");
    AddFileToDownloadsTable("materials/models/custom/player/meangreen/hvyweapon/eye-iris-red.vtf");
    AddFileToDownloadsTable("materials/models/custom/player/meangreen/hvyweapon/heavy_head.vtf");
    AddFileToDownloadsTable("materials/models/custom/player/meangreen/hvyweapon/heavy_head_blue.vmt");	
    AddFileToDownloadsTable("materials/models/custom/player/meangreen/hvyweapon/heavy_head_blue_invun.vmt");
    AddFileToDownloadsTable("materials/models/custom/player/meangreen/hvyweapon/heavy_head_blue_invun.vtf");
    AddFileToDownloadsTable("materials/models/custom/player/meangreen/hvyweapon/heavy_head_red.vmt");
    AddFileToDownloadsTable("materials/models/custom/player/meangreen/hvyweapon/heavy_head_red_invun.vmt");
    AddFileToDownloadsTable("materials/models/custom/player/meangreen/hvyweapon/heavy_head_red_invun.vtf ");
    AddFileToDownloadsTable("materials/models/custom/player/meangreen/hvyweapon/hvyweapon_blue.vmt");	
    AddFileToDownloadsTable("materials/models/custom/player/meangreen/hvyweapon/hvyweapon_blue.vtf");
    AddFileToDownloadsTable("materials/models/custom/player/meangreen/hvyweapon/hvyweapon_blue_invun.vmt");
    AddFileToDownloadsTable("materials/models/custom/player/meangreen/hvyweapon/hvyweapon_blue_invun.vtf");
    AddFileToDownloadsTable("materials/models/custom/player/meangreen/hvyweapon/hvyweapon_red.vmt");
    AddFileToDownloadsTable("materials/models/custom/player/meangreen/hvyweapon/hvyweapon_red.vtf");
    AddFileToDownloadsTable("materials/models/custom/player/meangreen/hvyweapon/hvyweapon_red_invun.vmt");
    ...
    AddFileToDownloadsTable("materials/models/custom/player/zombie/hvyweapon/heavy_head.vtf");
    AddFileToDownloadsTable("materials/models/custom/player/zombie/hvyweapon/heavy_head_blue.vmt");
    AddFileToDownloadsTable("materials/models/custom/player/zombie/hvyweapon/hvyweapon_blue.vmt");
    AddFileToDownloadsTable("materials/models/custom/player/zombie/hvyweapon/hvyweapon_blue.vtf");
    AddFileToDownloadsTable("materials/models/custom/player/zombie/hvyweapon/hvyweapon_hands.vmt");
    AddFileToDownloadsTable("materials/models/custom/player/zombie/hvyweapon/hvyweapon_hands.vtf");
	
    AddFileToDownloadsTable("models/custom/player/zombie/scout.mdl");
    AddFileToDownloadsTable("materials/models/custom/player/zombie/scout/scout_blue.vtf");
    AddFileToDownloadsTable("materials/models/custom/player/zombie/scout/scout_blue.vmt");
    AddFileToDownloadsTable("materials/models/custom/player/zombie/scout/scout_hands.vtf");
    AddFileToDownloadsTable("materials/models/custom/player/zombie/scout/scout_hands.vmt");
    AddFileToDownloadsTable("materials/models/custom/player/zombie/scout/scout_head.vtf");
    AddFileToDownloadsTable("materials/models/custom/player/zombie/scout/scout_head_blue.vmt");
    
	AddFileToDownloadsTable("models/custom/player/zombie/spy.mdl");
    AddFileToDownloadsTable("materials/models/custom/player/zombie/spy/spy_blue.vtf");
    AddFileToDownloadsTable("materials/models/custom/player/zombie/spy/spy_blue.vmt");
    AddFileToDownloadsTable("materials/models/custom/player/zombie/spy/spy_hands_blue.vtf");
    AddFileToDownloadsTable("materials/models/custom/player/zombie/spy/spy_hands_blue.vmt");
    AddFileToDownloadsTable("materials/models/custom/player/zombie/spy/spy_head_blue.vtf");
    AddFileToDownloadsTable("materials/models/custom/player/zombie/spy/spy_head_blue.vmt");
}
    ...
    if (!bBeginning)
    {
        ...
        for (new i = 1; i <= MaxClients; i++)
        {
            if (IsClientInGame(i))
            {
                ...
                if (isZom(i))
                {
                    CPrintToChat(i, "\x05[ZF]\x01 %t", "zombie are frenzied!");
                }
...
public Action:StopZombieRage(Handle:hTimer)
{
    ...
    if (roundState() == RoundActive) {
        for (new i = 1; i <= MaxClients; i++)
        {
            ...
            {
                CPrintToChat(i, "\x05[ZF]\x01 %t", "zombie are resting...");
            }
        }
    }
}
...
MusicHandleClient(iClient)
{
    ...
    else
    {
        /*
            ...
            Client health
            number of zombie surrounding him
            Zombie Rage
            ...
            Zombie calculation
            zombie within 10 meters are counted
            The total inverted distance of all the zombie. ie 10 for a zombie right up your face.
        ...
        */
        ...
        new Float:fzombie = GetZombieNumber(iClient);
        
        new Float:fScared = fzombie / fHealth + fRage * 20.0;
        
        /*
        if (IsMecha(iClient))
        {
            decl String:strInput[255];
            Format(strInput, sizeof(strInput), "zombie: %.1f\nHealth: %.1f\nScared: %.1f", fzombie, fHealth, fScared);
        ...
        */
        ...
        if (g_bRoundActive)
        {
            if (zombieHaveTank() && iMusic != MUSIC_GOO) iMusic = MUSIC_TANKMOOD;
...
ZombieTank(iCaller = 0)
{
    ...
    if (zombieHaveTank())
    {
        if (validClient(iCaller)) PrintToChat(iCaller, "%t", "zombie already have a tank");
        return;
    }
    if (g_iZombieTank > 0)
    {   
    ...
    if (g_bZombieRage)
    {
        if (validClient(iCaller)) PrintToChat(iCaller, "%t","zombie are frenzied");
        return;
    }
...
public Action:command_tank(client, args)
{
    ...
    if (zombieHaveTank()) return Plugin_Handled;
